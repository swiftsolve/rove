...
# Load Rove's AppArmor profile. On Ubuntu 24.04+/26.04 — and any distro with
...
if command -v apparmor_parser >/dev/null 2>&1 \
   ...
   && [ -f /etc/apparmor.d/rove ]; then
    apparmor_parser -r -W /etc/apparmor.d/rove 2>/dev/null || true
fi
