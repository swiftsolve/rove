...
export function useTheme(): Theme {
  ...
  // place rather than stacking transitions.
  const runningRef = useRef<RunningSwap | null>(null)
  ...
  // Flip the theme with a circular reveal: the incoming theme grows as a circle
  // from the centre of the window rather than the browser's default cross-fade.
  // The View Transitions API snapshots the old and new states; we then drive the
  // new snapshot's clip-path from a zero-radius circle out to one that covers the
  // whole window (radius = corner distance from centre). Engines without the API
  // (older WebKitGTK) and users who prefer reduced motion just get an instant
  // swap — the state change still applies, only the animation is skipped.
  const setMode = useCallback((nextMode: ThemeMode): void => {
    ...
    transition.finished.finally(() => {
      ...
      if (runningRef.current === swap) runningRef.current = null
    })
  }, [])
