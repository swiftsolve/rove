...
(`platform/mod.rs:61`), which uses the cross-platform `sysinfo` crate. It is
used when a native probe fails *and* as the whole implementation on any OS
...
- **Linux** (`platform/linux.rs:32`): `ip -j addr` gives JSON, parsed with serde.
  ...
  No command is needed; `/sys` is a virtual filesystem the kernel exposes.
...
`-n <count> -w <ms>`, Unix uses `-c <count> -i 0.2 -W 1`) and are abstracted
behind `ping_command` (`platform/mod.rs:154`).
